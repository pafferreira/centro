...
export const PasseRegistrationView: React.FC<PasseRegistrationViewProps> = ({ attendances, onAddAttendance, onNavigate }) => {
    ...
    const [attendancePhase, setAttendancePhase] = useState<AttendancePhase>(AttendancePhase.PrimeiraVez);
    ...
    return (
        <PageContainer>
            <Header title="Cadastro Dia de Passe" onHome={() => onNavigate('DASHBOARD')} />
                ...
                <form onSubmit={handleSubmit} className="bg-white p-5 rounded-2xl shadow-soft border border-slate-100 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <select
                            ...
                            onChange={e => setAttendancePhase(e.target.value as AttendancePhase)}
                            className="w-full px-4 py-3 bg-slate-50 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500/30"
                        >
                    ...
                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-semibold text-slate-700">Tipo de Passe</label>
                        <div className="flex gap-3">
                            <button
                                type="button"
                                onClick={() => setPasseType(PasseType.A1)}
                                className={`flex-1 py-3 rounded-xl font-bold border transition-colors ${passeType === PasseType.A1 ? 'bg-blue-100 border-blue-400 text-blue-800' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'
                                    }`}
                            >
                                A1 (Rápido)
                            </button>
                            <button
                                ...
                                className={`flex-1 py-3 rounded-xl font-bold border transition-colors ${passeType === PasseType.A2 ? 'bg-amber-100 border-amber-400 text-amber-800' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'
                                    }`}
                            >
                                A2 (Longo)
                            </button>
                    ...
                    <button type="submit" className="mt-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl shadow-md transition-all active:scale-[0.98]">
                        Adicionar à Fila
                    </button>
                </form>
