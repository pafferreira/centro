...
import { Worker, Assistido, FichaAssistencia, PasseAttendance, AttendancePhase, PasseType, AttendanceStatus, WorkerRole } from '../types';

interface AssistanceViewProps {
    workers: Worker[];
    assistido: Assistido;
    onSaveFicha: (ficha: FichaAssistencia, atendimentos: PasseAttendance[]) => void;
    onBack?: () => void;
    onHome?: () => void;
}

export const AssistanceView: React.FC<AssistanceViewProps> = ({ workers, assistido, onSaveFicha, onBack, onHome }) => {
        ...
        return (
            <div className="px-5 pb-5 flex flex-col gap-4 border-t border-slate-100 pt-4">
                ...
                <div className="mb-4">
                    ...
                    <div className="flex flex-col gap-0 bg-slate-50/80 rounded-[14px] shadow-inner border border-slate-100 divide-y divide-slate-200/60 overflow-hidden">
                        <div className="flex flex-row gap-0 divide-x divide-slate-200/60 w-full">
                            ...
                            <div className="flex items-center gap-2 shrink-0 px-3 py-3 w-1/3">
                        ...
                        <div className="flex items-center gap-2 px-3 py-3">
                            <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center shrink-0">
                                <CardsIcon className="w-4 h-4 text-indigo-600" />
                            </div>
                            <div className="flex flex-col w-full">
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-0.5">Resultado da Entrevista (Sequência)</span>
                                <div className="flex items-center gap-2 mt-0.5">
                                    {formBase.qtdA2 > 0 && (
                                        <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-xs font-bold rounded-md border border-amber-200 shadow-sm">{formBase.qtdA2}x Passes A2</span>
                                    )}
                                    {formBase.qtdA2 > 0 && formBase.qtdA1 > 0 && (
                                        <span className="text-slate-400 font-bold text-xs">+</span>
                                    )}
                                    {formBase.qtdA1 > 0 && (
                                        <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-bold rounded-md border border-blue-200 shadow-sm">{formBase.qtdA1}x Passes A1</span>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            <div className="mt-4 flex flex-col gap-4 sm:gap-5 px-1 pb-10">
                ...
                <button
                    onClick={() => {
                        ...
                        const ent = workers.find(w => w.name === initialForm.entrevistador);
                        ...
                        const ficha: FichaAssistencia = {
                            ...
                            dataEntrevista: initialForm.dataEntrevista,
                            qtdA2: initialForm.qtdA2,
                            qtdA1: initialForm.qtdA1,
                            tipoFicha: 'Inicial',
                            statusFicha: 'Ativa'
                        };

                        const ats: PasseAttendance[] = [];
                        let sessionCount = 0;
                        for (let i = 0; i < 8; i++) {
                            const dateStr = initialDates[i];
                            if (dateStr) {
                                sessionCount++;
                                const isA2 = sessionCount <= initialForm.qtdA2;
                                ats.push({
                                    id: crypto.randomUUID(),
                                    date: dateStr,
                                    assistidoId: assistido.id,
                                    assistidoName: assistido.nome,
                                    passeType: isA2 ? PasseType.A2 : PasseType.A1,
                                    attendancePhase: sessionCount === 1 ? AttendancePhase.PrimeiraVez : AttendancePhase.Normal,
                                    status: AttendanceStatus.Aguardando,
                                    // A property like allocatedRoomId might be added later by the distribution logic.
                                });
                            }
                        }

                        // Treat the very last session as 'Retorno' if it reached 8 (or total sessions)
                        if (ats.length > 0 && sessionCount === initialForm.qtdA1 + initialForm.qtdA2) {
                            ats[ats.length - 1].attendancePhase = AttendancePhase.Retorno;
                        }

                        onSaveFicha(ficha, ats);
                    }}
                    className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-4 rounded-2xl shadow-lg transition-all active:scale-[0.98]"
                >
